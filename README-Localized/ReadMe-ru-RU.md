# <a name="office-add-in-that-uses-the-oauthio-service-to-get-authorization-to-external-services"></a>Надстройка Office, которая использует службу OAuth.io для авторизации в случае внешних служб
...
3. Создайте учетные данные приложения для проекта и в качестве типа учетных данных выберите **Идентификатор клиента OAuth**, а в качестве типа приложения — **Веб-приложение**. В качестве **имени** клиента (и имени продукта, если появится соответствующий запрос) укажите "Office-Add-in-OAuth.io".
...
2. Добавьте **Google+** в качестве поставщика, или интегрированного API, для приложения. Введите идентификатор клиента и секрет клиента, полученные при создании проекта Google, а затем укажите `profile` в качестве **области**. Если конфигурация поставщика (интегрированный API) не сохраняется в Internet Explorer, попробуйте повторно открыть информационную панель OAuth.io в другом браузере.
3. Повторите предыдущее действие для **Facebook**, но в качестве **области** укажите `user_about_me`.

## <a name="add-the-public-key-to-the-sample-code"></a>Добавление в пример кода открытого ключа
...
6. На вкладке "Главная" отобразится новая группа под названием **OAuth.io** с кнопкой **Показать** и значком. Нажмите эту кнопку, чтобы открыть надстройку.
